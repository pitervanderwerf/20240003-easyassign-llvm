          ...
          nodes_map(network.fetchJunctionsFromDatabase())
{
    if (Config::LEVEL_OF_DETAIL != "C2C") {
        ...
        //predecessor_maps.resize(35357);
    }
    else
    {
        //predecessor_maps.resize(Config::NUMBER_OF_CENTROIDS);
    }

    //link_flows.reserve(network.links_map.size());
    //for (const auto& link : network.links_map) {
    //    link_flows[link.first] = { 0.0, 0.0 };
    //}
}
...
//
//    std::unordered_map<int, LinkFlow> local_link_flows;
//    local_link_flows.reserve(network.links_map.size() / 32);
//    std::unordered_map<int, LinkFlow> local_link_flows_optimal;
...
//
//        // Filter the target vertices based on total_od_flow threshold
//        std::unordered_set<int> relevant_targets;
...
//                    relevant_targets.insert(target_vertex_id);
//                }
//            }
//        }
//
//        // Modify Dijkstra to stop early when all relevant targets are found
//        try {
//            EarlyStopVisitor visitor(relevant_targets, vertex_to_node);
...
//                            .visitor(visitor)
//            );
//        } catch (const FoundAllTargets&) {
//            // Early stop when all targets are found
//        }
//
//        if (Config::ASSIGN_FLOWS || Config::CREATE_SKIMS || Config::PRINT_PATHS) {
//            for (int target_vertex_id : relevant_targets) {
//                double total_od_flow = odm.at(source_vertex_id - 1, target_vertex_id - 1);
...
void Assignment::assign(int start_source_vertex, int end_source_vertex) {
    ...
    // Create an unordered_map for vertex_to_node for faster lookups
    std::unordered_map<Vertex, int> vertex_to_node_unordered(vertex_to_node.begin(), vertex_to_node.end());

    for (int source_vertex_id = start_source_vertex; source_vertex_id <= end_source_vertex; ++source_vertex_id) {
        Vertex source_vertex = node_to_vertex[source_vertex_id];

        // Filter the target vertices based on total_od_flow threshold
        std::unordered_set<Vertex> relevant_targets;
        for (int target_vertex_id = 1; target_vertex_id <= Config::NUMBER_OF_CENTROIDS; ++target_vertex_id) {
            if (source_vertex_id != target_vertex_id) {
                double total_od_flow = odm.at(source_vertex_id - 1, target_vertex_id - 1);
                if (total_od_flow >= Config::INSIGNIFICANT_OD_PAIRS_THRESHOLD) {
                    relevant_targets.insert(node_to_vertex[target_vertex_id]);
                }
            }
        }
        ...
        // Modify Dijkstra to stop early when all relevant targets are found
//        try {
//            EarlyStopVisitor visitor(relevant_targets);
//            dijkstra_shortest_paths_no_color_map(
//                    G,
//                    source_vertex,
//                    predecessor_map(&predecessors[0])
...
//        }
        ...
        // Update progress
        counter++;
        if (counter % 1000 == 0 || counter == Config::NUMBER_OF_CENTROIDS) {
            std::lock_guard<std::mutex> lock(counter_mutex);
            int progress = int(static_cast<double>(counter) / Config::NUMBER_OF_CENTROIDS * 100);
            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(Config::NUMBER_OF_CENTROIDS) + " \t /// Progress: " + std::to_string(progress) + "%");
        }
...
void Assignment::assignPc5(int start_source_vertex, int end_source_vertex, int number_of_pc5_nodes) {
    std::vector<double> distances(num_vertices(G));
    std::vector<Vertex> predecessors(num_vertices(G));
    ...
    std::unordered_map<int, LinkFlow> local_link_flows;
    local_link_flows.reserve(network.links_map.size() / 256);

    std::map<std::pair<int, int>, double> local_skim_cost_list;
    // Define the check interval for the visitor
    const size_t check_interval = 100;  // Adjust this value as needed

    for (int source_vertex_id = start_source_vertex; source_vertex_id <= end_source_vertex; ++source_vertex_id) {
        Vertex source_vertex = node_to_vertex[source_vertex_id];

        // Initialize target_flags vector to false
        std::vector<bool> target_flags(num_vertices(G), false);
        size_t relevant_target_count = 0;

        // Filter the target vertices based on total_od_flow threshold
        for (int target_vertex_id = 1; target_vertex_id <= number_of_pc5_nodes; ++target_vertex_id) {
            if (source_vertex_id != target_vertex_id) {
                double total_od_flow = odm.at(network.pc_to_centroid_map[source_vertex_id] - 1,
                                              network.pc_to_centroid_map[target_vertex_id] - 1);
                if (total_od_flow >= Config::INSIGNIFICANT_OD_PAIRS_THRESHOLD) {
                    Vertex target_vertex = node_to_vertex[target_vertex_id];
                    target_flags[target_vertex] = true;
        ...
        // Modify Dijkstra to stop early when all relevant targets are found
        try {
            EarlyStopVisitor visitor(target_flags, relevant_target_count, check_interval);
            dijkstra_shortest_paths_no_color_map(
                    G,
                    source_vertex,
                    predecessor_map(&predecessors[0])
        ...
        if (Config::ASSIGN_FLOWS || Config::CREATE_SKIMS || Config::PRINT_PATHS) {
            for (int target_vertex_id = 1; target_vertex_id <= number_of_pc5_nodes; ++target_vertex_id) {
                if (source_vertex_id != target_vertex_id) {
                    double od_flow = odm.at(network.pc_to_centroid_map[source_vertex_id] - 1,
                                            network.pc_to_centroid_map[target_vertex_id] - 1) *
                                     network.node_fractions[source_vertex_id] *
                                     network.node_fractions[target_vertex_id] * va_factor;

                    if (od_flow >= Config::INSIGNIFICANT_OD_PAIRS_THRESHOLD) {
                        Vertex v = node_to_vertex[target_vertex_id];
                        double path_cost = 0;
                        ...
                        while (v != source_vertex) {
                            ...
                            if (edge_pair.second) {  // Check if the edge exists
                                ...
                                if (Config::ASSIGN_FLOWS) {
                                    auto &link_flow = local_link_flows[edge_props.link_id]; // This will default initialize if not exists
        ...
        // Update progress
        counter++;
        if (counter % 1000 == 0 || counter == number_of_pc5_nodes) {
            std::lock_guard<std::mutex> lock(counter_mutex);
            int progress = int(static_cast<double>(counter) / number_of_pc5_nodes * 100);
            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(number_of_pc5_nodes) + " \t /// Progress: " + std::to_string(progress) + "%");
        }
    }
...
//                            //    local_paths += std::to_string(vertex_to_node[v]) + ";";
//                            //}
//
//                            while (v != source_vertex) {
//                                Vertex prev = predecessors[v];
...
//
//                                    //if (Config::ASSIGN_FLOWS) {
//                                    //    local_link_flows[edge_props.link_id][edge_props.direction] += od_flow;
//                                    //}
//
//                                    if (Config::ASSIGN_FLOWS) {
//                                        auto &link_flow = local_link_flows[edge_props.link_id]; // This will default initialize if not exists
//
//                                        if (edge_props.direction ==
//                                            0) { // Assuming FORWARD_DIRECTION is defined appropriately
//                                            link_flow.flow_forward += od_flow;
//                                        } else if (edge_props.direction ==
//                                                   1) { // Assuming BACKWARD_DIRECTION is defined appropriately
//                                            link_flow.flow_backward += od_flow;
//                                        }
//                                    }
//
//                                    if (Config::PRINT_PATHS) {
//                                        //local_paths += std::to_string(link_id) + ";";
//                                    }
//
//                                    if (Config::CREATE_SKIMS) {
//                                        double cost = edge_props.weight;
...
//                            }
//
//                            //if (Config::PRINT_PATHS && !local_paths.empty()) {
//                            //    local_paths += std::to_string(vertex_to_node[source_vertex]) + "\n";
//                            //}
//                        }
//                    }
//                }
...
//            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(number_of_pc5_nodes) + " \t /// Progress: " + std::to_string(progress) + "%");
//        }
//        if (counter == number_of_pc5_nodes) {
//            std::lock_guard<std::mutex> lock(counter_mutex);
//            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(number_of_pc5_nodes) + " \t /// Progress: " + std::to_string(100) + "%\n");
//        }
//    }
//
//    // Flush local_link_flows to shared link_flows
...
void Assignment::assignPc6(int start_source_vertex, int end_source_vertex, int number_of_pc6_nodes) {
    std::vector<double> distances(num_vertices(G));
    std::vector<Vertex> predecessors(num_vertices(G));

    auto weight_map = get(&EdgeProperties::weight, G);

    //std::unordered_map<int, std::array<double, 2>> local_link_flows;
    std::unordered_map<int, LinkFlow> local_link_flows;
    local_link_flows.reserve(network.links_map.size() / 256);
    ...
    for (int source_vertex_id = start_source_vertex; source_vertex_id <= end_source_vertex; ++source_vertex_id) {
        ...
        if (Config::ASSIGN_FLOWS || Config::CREATE_SKIMS || Config::PRINT_PATHS) {
            for (int target_vertex_id = 1; target_vertex_id <= number_of_pc6_nodes; target_vertex_id++) {
                if (source_vertex_id != target_vertex_id) {
                    double od_flow = (odm.at(network.pc_to_centroid_map[source_vertex_id] - 1, network.pc_to_centroid_map[target_vertex_id] - 1) * network.node_fractions[source_vertex_id] * network.node_fractions[target_vertex_id]) * va_factor;
                    ...
                    if (od_flow >= Config::INSIGNIFICANT_OD_PAIRS_THRESHOLD) {
                        ...
                        while (v != source_vertex) {
                            ...
                            if (Config::ASSIGN_FLOWS) {
                                ...
                                if (edge_props.direction == 0) { // Assuming FORWARD_DIRECTION is defined appropriately
                                    link_flow.flow_forward += od_flow;
                                }
                                else if (edge_props.direction == 1) { // Assuming BACKWARD_DIRECTION is defined appropriately
                                    link_flow.flow_backward += od_flow;
                                }
                            }

                            //if (Config::CREATE_SKIMS) {
                            //     cost = edge_props.weight;
                            //     path_cost += cost;
                            //}
        ...
        if (counter % 1000 == 0) {
            std::lock_guard<std::mutex> lock(counter_mutex);
            int progress = int(static_cast<double>(counter) / number_of_pc6_nodes * 100);
            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(number_of_pc6_nodes) + " \t /// Progress: " + std::to_string(progress) + "%");
        }
        if (counter == number_of_pc6_nodes) {
            std::lock_guard<std::mutex> lock(counter_mutex);
            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(number_of_pc6_nodes) + " \t /// Progress: " + std::to_string(100) + "%\n");
        }
    }
...
//void Assignment::updateFlowsFromGraph() {
//    for (auto edgePair = edges(G); edgePair.first != edgePair.second; ++edgePair.first) {
//        Edge e = *edgePair.first;
//        EdgeProperties props = G[e];  // Assuming G is your graph and it can return EdgeProperties for an edge
//
//        if (this->iteration == 1) {
//            link_flows[props.link_id].flow_forward = (props.direction ? props.flow : 0.0);
//            link_flows[props.link_id].flow_backward = (props.direction ? 0.0 : props.flow);
//        }
//        else {
//            link_flows[props.link_id].flow_forward += (props.direction ? props.flow : 0.0);
//            link_flows[props.link_id].flow_backward += (props.direction ? 0.0 : props.flow);
...
void Assignment::executePc5(int number_of_pc5_nodes)
{
    Stopwatch stopwatch("ASSIGNMENT");
    stopwatch.start();
    ...
    for (int i = 1; i <= Config::NUMBER_OF_ITERATIONS; i++) {
        this->iteration = i;
        this->buildGraphFromLinksMapWithTurns();

        if (iteration > 1) {
            network.reducePreviousLinkFlows(this->iteration);
        }

        if (Config::ASSIGNMENT_METHOD != "AON" && this->iteration == 1) {
            odm.setOdMatrixFromBinary();
        }

        std::vector<std::thread> threads;
        const int CHUNK_SIZE = 32;
        const int total_tasks = number_of_pc5_nodes;
        std::atomic<int> next_task(1); // Start from 1
        ...
        for (int i = 0; i < Config::MAX_THREADS; ++i) {
            threads.emplace_back([&, i]() {
                while (true) {
                    ...
                    int start_source_vertex = task_id;
                    int end_source_vertex = std::min(task_id + CHUNK_SIZE - 1, number_of_pc5_nodes);

                    assignPc5(start_source_vertex, end_source_vertex, number_of_pc5_nodes);
                }
            });
        }
        ...
        this->dimensions.i = this->iteration;
        skm_cost.setPmturi({ dimensions.p, dimensions.m, dimensions.t, dimensions.u, dimensions.r[0], this->iteration });
        skm_dist.setPmturi({ dimensions.p, dimensions.m, dimensions.t, dimensions.u, dimensions.r[1], this->iteration });
        skm_time.setPmturi({ dimensions.p, dimensions.m, dimensions.t, dimensions.u, dimensions.r[2], this->iteration });

        (Config::ASSIGN_FLOWS) ? this->flowsToLinks() : void();
        if (this->iteration == Config::NUMBER_OF_ITERATIONS) {
            //(Config::ASSIGN_FLOWS) ? this->printFlows() : void();
            (Config::ASSIGN_FLOWS) ? this->printFlowsLinksMap() : void();
            (Config::ASSIGN_FLOWS) ? this->streamToDatabaseLinksMap() : void();
            //(Config::CREATE_SKIMS) ? this->skm_cost.writeToBinaryFile() : 0;
            //(Config::CREATE_SKIMS) ? this->skm_dist.writeToBinaryFile() : 0;
            //(Config::CREATE_SKIMS) ? this->skm_time.writeToBinaryFile() : 0;
            (Config::CREATE_SKIMS) ? this->skm_cost.writeMatrixToTextFile("results/skm_cost.txt") : void();
            //(Config::CREATE_SKIMS) ? this->skm_dist.writeMatrixToTextFile("skm_dist.txt") : void();
            //(Config::CREATE_SKIMS) ? this->skm_time.writeMatrixToTextFile("skm_time.txt") : void();
            //(Config::PRINT_PATHS) ? this->printPaths() : void();
        }

        //network.printLinksToFile("00_network_before_updateCosts.txt");
        //network.printLinksToFile("01_network_after_updateCosts.txt");

        //previous_link_flows = link_flows;
        resetLinkFlows();

        skm_cost.printMatrixSum();
        skm_time.printMatrixSum();
        skm_cost.setMatrixSum(0);
        skm_dist.setMatrixSum(0);
        skm_time.setMatrixSum(0);

        skm_cost.resetMatrix();
        skm_dist.resetMatrix();
        skm_time.resetMatrix();

        counter = 0;

        network.updateCosts();

        stopwatch.checkpoint("ITERATION " + std::to_string(this->iteration) + " DONE");
    }
...
void Assignment::executePc6(int number_of_pc6_nodes)
{
    Stopwatch stopwatch("ASSIGNMENT");
    stopwatch.start();
    ...
    for (int i = 1; i <= Config::NUMBER_OF_ITERATIONS; i++) {
        ...
        std::vector<std::thread> threads;
        const int CHUNK_SIZE = 32;
        const int total_tasks = number_of_pc6_nodes;
        std::atomic<int> next_task(1); // Start from 1
        ...
        for (int i = 0; i < Config::MAX_THREADS; ++i) {
            threads.emplace_back([&, i]() {
                while (true) {
                    ...
                    int start_source_vertex = task_id;
                    int end_source_vertex = std::min(task_id + CHUNK_SIZE - 1, number_of_pc6_nodes);

                    assignPc6(start_source_vertex, end_source_vertex, number_of_pc6_nodes);
                }
            });
        }
        ...
        stopwatch.checkpoint("ITERATION " + std::to_string(this->iteration));
    }
...
//
//void Assignment::assignPc5(int start_source_vertex, int end_source_vertex) {
//    int pc5_nodes = 35357;
//
//    std::vector<double> distances(num_vertices(G));
//    std::vector<Vertex> predecessors(num_vertices(G));
...
//
//        if (Config::ASSIGN_FLOWS || Config::CREATE_SKIMS || Config::PRINT_PATHS) {
//            for (int target_vertex_id = 1; target_vertex_id <= pc5_nodes; target_vertex_id++) {
//                if (source_vertex_id != target_vertex_id) {
//                    double od_flow = (odm.at(network.pc_to_centroid_map[source_vertex_id] - 1, network.pc_to_centroid_map[target_vertex_id] - 1) * network.node_fractions[source_vertex_id] * network.node_fractions[target_vertex_id]) * va_factor;
//
//                    if (od_flow >= Config::INSIGNIFICANT_OD_PAIRS_THRESHOLD) {
...
//        if (counter % 1000 == 0) {
//            std::lock_guard<std::mutex> lock(counter_mutex);
//            int progress = int(static_cast<double>(counter) / pc5_nodes * 100);
//            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(pc5_nodes) + " \t /// Progress: " + std::to_string(progress) + "%");
//        }
//        if (counter == pc5_nodes) {
//            std::lock_guard<std::mutex> lock(counter_mutex);
//            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(pc5_nodes) + " \t /// Progress: " + std::to_string(100) + "%\n");
//        }
//    }
//
//    // Flush local_link_flows to shared link_flows
...
//    }
//
//    //// Flush local paths to shared paths
//    //if (Config::PRINT_PATHS && !local_paths.empty()) {
//    //    std::lock_guard<std::mutex> lock(paths_file_mutex);
//    //    paths += local_paths;
//    //}
//}
//
//void Assignment::assignPc6(int start_source_vertex, int end_source_vertex) {
//    int pc6_nodes = 462369;
//
//    std::vector<double> distances(num_vertices(G));
//    std::vector<Vertex> predecessors(num_vertices(G));
//
...
//
//        if (Config::ASSIGN_FLOWS || Config::CREATE_SKIMS || Config::PRINT_PATHS) {
//            for (int target_vertex_id = 1; target_vertex_id <= pc6_nodes; target_vertex_id++) {
//                if (source_vertex_id != target_vertex_id) {
//                    double od_flow = (odm.at(network.pc_to_centroid_map[source_vertex_id] - 1, network.pc_to_centroid_map[target_vertex_id] - 1) * network.node_fractions[source_vertex_id] * network.node_fractions[target_vertex_id]) * va_factor;
//
//                    if (od_flow >= Config::INSIGNIFICANT_OD_PAIRS_THRESHOLD) {
...
//        if (counter % 1000 == 0) {
//            std::lock_guard<std::mutex> lock(counter_mutex);
//            int progress = int(static_cast<double>(counter) / pc6_nodes * 100);
//            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(pc6_nodes) + " \t /// Progress: " + std::to_string(progress) + "%");
//        }
//        if (counter == pc6_nodes) {
//            std::lock_guard<std::mutex> lock(counter_mutex);
//            Logger::log("\t" + std::to_string(counter) + " centroids done of " + std::to_string(pc6_nodes) + " \t /// Progress: " + std::to_string(100) + "%\n");
//        }
//    }
//
//    // Flush local_link_flows to shared link_flows
...
//    Logger::log("\tTotal traveltime spent        : " + std::to_string(total_traveltime_spent));
//    Logger::log("\tTotal traveltime spent optimal: " + std::to_string(total_traveltime_spent_optimal));
//    Logger::log("\tDuality gap: " + std::to_string(total_traveltime_spent - total_traveltime_spent_optimal));
//    Logger::log("\tRelative gap: " + std::to_string((total_traveltime_spent - total_traveltime_spent_optimal) / total_traveltime_spent_optimal));
//}
//
////void Assignment::updateFlowsFromGraph() {
////    for (auto edgePair = edges(G); edgePair.first != edgePair.second; ++edgePair.first) {
...
//        std::vector<std::thread> threads;
//        const int CHUNK_SIZE = 32;
//        const int total_tasks = pc5_nodes;
//        std::atomic<int> next_task(1); // Start from 1
//
//        // Launch worker threads
//        for (int i = 0; i < Config::MAX_THREADS; ++i) {
...
//
//                    int start_source_vertex = task_id;
//                    int end_source_vertex = std::min(task_id + CHUNK_SIZE - 1, pc5_nodes);
//
//                    assignPc5(start_source_vertex, end_source_vertex);
//                }
//                });
//        }
//
...
//        std::vector<std::thread> threads;
//        const int CHUNK_SIZE = 32;
//        const int total_tasks = pc6_nodes;
//        std::atomic<int> next_task(1); // Start from 1
//
//        // Launch worker threads
//        for (int i = 0; i < Config::MAX_THREADS; ++i) {
...
//
//                    int start_source_vertex = task_id;
//                    int end_source_vertex = std::min(task_id + CHUNK_SIZE - 1, pc6_nodes);
//
//                    assignPc6(start_source_vertex, end_source_vertex);
//                }
//                });
//        }
//
